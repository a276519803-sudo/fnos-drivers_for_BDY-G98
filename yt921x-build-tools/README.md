...
| `deploy_full_yt921x.sh` | 部署官方完整版驱动（推荐，含高级功能） |
| `README.md` | 本说明文档 |
...
**推荐使用官方完整版**，只需系统中保留了旧版本内核的驱动文件即可。
...
官方完整版驱动已适配 0x9988，可直接使用。源码编译版也已强制将 `ETH_P_YT921X` 替换为 0x9988。

## 注意事项

### 关于 tag_yt921x.ko

`tag_yt921x.ko`（标签协议驱动）需要 DSA 子系统的内部头文件，飞牛的内核头文件包不包含这些内部文件。官方完整版部署脚本会直接从旧内核目录复制已编译好的 `tag_yt921x.ko`。
...
- **v1.0.4**：新增官方完整版驱动部署脚本 `deploy_full_yt921x.sh`，支持硬件流卸载、QoS、DSA 高级功能
